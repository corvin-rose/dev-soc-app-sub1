...
class IntListTest extends AnyFunSuite {
  ...
  test("testInsertSorted4") {
    assert(SinglyLinkedIntList(1, 2, 4, 5, 6).insertSorted(3) === SinglyLinkedIntList(1, 2, 3, 4, 5, 6))
  }
  test("testInsertSorted5") {
    assert(SinglyLinkedIntList(2, 3, 4, 5, 6).insertSorted(1) === SinglyLinkedIntList(1, 2, 3, 4, 5, 6))
  }
}