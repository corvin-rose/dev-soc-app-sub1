...
abstract class SinglyLinkedIntList extends IntList {
  ...
  override def reduceLeft(reduceFunc: (Int, Int) => Int): Int = tail match {
    case Empty => head
    case _ => Cons(reduceFunc(head, tail.head), tail.tail).reduceLeft(reduceFunc)
  }

  override def foldRight(initial: Int)(reduceFunc: (Int, Int) => Int): Int = this match {
    case Empty => initial
    case _ => reduceFunc(head, tail.foldRight(initial)(reduceFunc))
  }
  ...
  override def insertSorted(elem: Int): IntList = this match {
    ...
    case Cons(head, tail) =>
      if (head >= elem) Cons(elem, Cons(head, tail))
